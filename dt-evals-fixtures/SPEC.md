...
- Every supported evaluator has **2 conversations** — one that clearly *passes*
  and one that clearly *fails/violates* — so both directions of the score are
  asserted.
- Emitted spans carry the attributes each targeted evaluator needs (input,
  ...
  name).
- The dataset includes **genuine multi-turn conversations** — one span per turn,
  linked by a shared `gen_ai.conversation.id` — mirroring how production chatbots
  emit spans in the wild (see §3.5). Multi-turn is genuine where the source
  provides it (toxicity, fluency); other evaluators use a 1-turn conversation.
- Content is drawn from **real, permissively-licensed evaluation datasets**
  (NVIDIA Nemotron-PII / HelpSteer2, Anthropic hh-rlhf, TruthfulQA, SummEval, …)
  where redistribution is allowed, and **self-authored** where it is not — see
  §3.7 for the split and the rationale.
...
├── SPEC.md                     # this file
└── opentelemetry/
    ├── app.py                  # FastAPI app; startup Traceloop init; /ingest route
    ├── tracing.py              # Traceloop/LangChain init, conv-id SpanProcessor, custom attrs
    ├── fixture_model.py        # FakeListChatModel subclass that sets model name + token usage
    ├── fixtures.py             # load/normalize fixtures.json into Case/Turn models
    ├── ingest.py               # replay a fixture as linked multi-turn spans
    ├── validate.py             # structural well-formedness checks for cases (§7)
    ├── build_fixtures.py       # dataset rows / authored content → curated fixtures.json (§3.7)
    ...
    ├── Makefile                # install / run / request / build-fixtures / test / help
    ├── .env.sample             # DT_ENDPOINT + DT_API_TOKEN template
    ├── README.md               # how to run + fixture format
    └── tests/                  # emitter, builders, validator, app (in-memory exporter)
```
