...
|---|---|---|
| **Turn-level correlation** | `_stamp_turn_io()` | Stamps the whole turn onto the `@workflow` span so one clean `input -> output` record exists, using the message form (`gen_ai.input.messages` / `gen_ai.output.messages`) from the Dynatrace semantic dictionary plus `gen_ai.operation.name=chat`. Only the message form is set: the Prompts renderer appends both the flat form (`gen_ai.prompt.N` / `gen_ai.completion.0`) and the message form, so setting both would render the same turn twice. It also sets `gen_ai.system` / `gen_ai.provider.name`, because the AI Observability app only treats a span as a GenAI span when one of those is present (`isNotNull(gen_ai.system) or isNotNull(gen_ai.provider.name)`); without them the `@workflow` span is filtered out of the Prompts view. |
