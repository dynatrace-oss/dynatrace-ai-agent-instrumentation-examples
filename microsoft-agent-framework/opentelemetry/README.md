...
Logs are exported too, and are trace-correlated so they appear on the span they were emitted inside. The demo logs **lifecycle events only** — which workflow ran, which agents, which tool, how many outputs — and deliberately not the conversation. Prompts and completions already travel as span attributes (`gen_ai.input.messages` / `gen_ai.output.messages`), which is the form the Dynatrace GenAI app queries; logging them again would duplicate sensitive content into a second signal with its own retention and permission rules for no analytical gain. The transcript still prints to the console for whoever is running the demo. Copy this split if you use the example as a starting point. Dynatrace does not synthesize `log.source` for OTLP log ingest, so records arrive with that field empty; `make run-collector` fills it in with a `transform` processor, using the emitting scope name. On a plain `make run` the field stays empty. Note `log.source` is permission-relevant in Dynatrace. One caveat worth knowing: `configure_otel_providers()` attaches its OTel logging handler to the **`agent_framework` logger only**, so anything logged outside that namespace never becomes a log record. The demo logs under `agent_framework.demo` for that reason — copy that pattern if you add logging of your own.
