...
2. Select **Spans**.
3. Click **Add pipeline**, name it `mistral-openinference-ai-spans`, and add processors matching the definitions in [`openpipeline-openinference.yaml`](openpipeline-openinference.yaml).
4. Go to the **Routing** tab and add an entry:
    - Matcher: `isNotNull(openinference.span.kind) AND service.name == "mistral/openinference-openpipeline"`
    - Pipeline: `mistral-openinference-ai-spans`
...
> **Note on request parameters:** `MistralAIInstrumentor` (like `GroqInstrumentor`) never emits discrete `llm.temperature` / `llm.max_tokens` / `llm.top_p` span attributes -- only a single `llm.invocation_parameters` JSON string. Option A's collector parses it with OTTL's `ParseJSON` (see `transform/pre_normalize` in [`otel-collector-config.yaml`](otel-collector-config.yaml), which extracts `temperature` only); Option B's OpenPipeline parses it with DQL's `jsonField()` (see the `openinference-request-params` processor in [`openpipeline-openinference.yaml`](openpipeline-openinference.yaml), which extracts all three). A naive `fieldsRename` from the flat attribute names -- the pattern this section's table implies and that some sibling OpenInference examples in this repo still use -- never matches anything, since those flat attributes don't exist.
...
To close these locally, add statements to the `transform` processor in [`otel-collector-config.yaml`](otel-collector-config.yaml). Option B (OpenPipeline) already maps all three request parameters server-side.
...
`MistralAIInstrumentor` wraps `mistralai`'s Chat Completion API (`Chat.complete`/`stream`) and Agents API (`Agents.complete`/`stream`) -- both tag the span as `openinference.span.kind = LLM` -- but it does not wrap `mistralai`'s embeddings, classifiers, or OCR endpoints, so neither option's pipeline needs to handle `embedding.*` OpenInference attributes -- unlike [`openai/openinference`](../../openai/openinference/), whose `OpenAIInstrumentor` also covers the embeddings endpoint.
...
- Option B: confirm the OpenPipeline routing entry is active; go to **Settings -> OpenPipeline -> Spans** in Dynatrace and verify the `mistral-openinference-ai-spans` pipeline is enabled and the routing matcher is `isNotNull(openinference.span.kind) AND service.name == "mistral/openinference-openpipeline"`.
