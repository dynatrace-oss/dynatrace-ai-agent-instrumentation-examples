...
- **Provider**: Ollama (local server, `llama3.2` model, default `http://localhost:11434`). No external cloud provider.
- **OTel setup**: No application-level OTel configuration. The app is a minimal FastAPI service. Instrumentation comes from the Dynatrace OneAgent injected at the pod level. OneAgent provides auto-instrumentation via the experimental **Ollama** sensor (Settings → OneAgent features → search "Ollama"). The **Python FastAPI** sensor must also be enabled for HTTP entry-point spans. Experimental sensors are best-effort and not covered by DT support SLAs; attribute collection and schema may change without notice.

## Verdict: PARTIAL

| Check | Status | Detail |
|-------|--------|--------|
| Provider identity (`must_have_any`) | ✅ | Expected via experimental Ollama sensor — `gen_ai.system` or `gen_ai.provider.name` captured automatically |
| `service.name` | ✅ | Set by OneAgent from K8s pod/deployment metadata |
| `gen_ai.request.model` | ✅ | Expected via experimental Ollama sensor |
| `gen_ai.response.model` | ✅ | Expected via experimental Ollama sensor |
| `gen_ai.usage.input_tokens` | ✅ | Expected via experimental Ollama sensor |
| `gen_ai.usage.output_tokens` | ✅ | Expected via experimental Ollama sensor |

## App view coverage

| View | Status | Root cause |
|------|--------|------------|
| All GenAI views gate | ✅ | Provider identity captured by experimental Ollama sensor |
...
|----------------|------------|--------------------|
| All GenAI spans | ✅ Expected | Provider identity captured by experimental Ollama sensor |
...
|-----------|---------|----------------|
| `gen_ai.input.messages` / `gen_ai.output.messages` | — | Prompt content not captured; experimental Ollama sensor does not support prompt capture |
...
**1. Enable the experimental Ollama sensor in OneAgent features**

Go to Settings → OneAgent features → search "Ollama" → enable the experimental sensor. Restart the monitored process. The sensor is best-effort; attribute collection and schema may change without notice and is not covered by DT support SLAs.
...
Prompt capture is not available via the experimental Ollama sensor. To capture message content, wrap `client.chat()` in a manual OTel span:
...
For token extraction, note that the Ollama response uses non-standard field names distinct from other providers:

```python
span.set_attribute("gen_ai.usage.input_tokens", response.prompt_eval_count)   # not usage.prompt_tokens
span.set_attribute("gen_ai.usage.output_tokens", response.eval_count)         # not usage.completion_tokens
