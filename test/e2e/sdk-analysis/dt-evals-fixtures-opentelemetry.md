...
3. **The env vars `OTEL_SEMCONV_STABILITY_OPT_IN` / `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT` are effectively no-ops for this instrumentor.** At v0.62.1 the LangChain instrumentation does not read them — modern messages are emitted whenever content capture is on (Traceloop's own `TRACELOOP_TRACE_CONTENT`, default true). They do no harm and document intent, but the modern-message behavior does not depend on them; the comment in `tracing.py` slightly overstates their effect.
4. **`gen_ai.agent.name` / `llm.request.type` / `gen_ai.request.temperature` absent** — expected for a plain chat invoke; only worth adding if the fixtures are extended to exercise agent or parameterized-request views.
